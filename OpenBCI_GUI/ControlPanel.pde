...
class ControlPanel {
    ...
    ControlPanel(OpenBCI_GUI mainClass) {
        ...
        esp32Box = new ESP32Box(x + w, dataSourceBox.y, w, h, globalPadding);
    ...
    public void draw() {
        ...
        if (systemMode != 10) { // only draw control panel boxes if system running is false
            ...
            } else if (eegDataSource == DATASOURCE_ESP32) {
                esp32Box.draw();
            }
        }
        ...
        sourceList.addCallback(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                if (theEvent.getAction() == ControlP5.ACTION_BROADCAST) {
                    ...
                    // ...and add this new block right after it.
                    else if (eegDataSource == DATASOURCE_ESP32) {
                        // This tells the GUI to show the serial port selection UI
                        selectedProtocol = BoardProtocol.SERIAL;
                        // This refreshes the list of available COM ports for the user
        ...
        serialList.addCallback(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                if (theEvent.getAction() == ControlP5.ACTION_BROADCAST) {
                    ...
                    output("ControlPanel: Selected Cognisync Port " + openBCI_portName);
                }
            }
        });
    }
...
class SessionDataBox {
    ...
    private String odfMessage = "Output has been set to Cognisync Data Format (CSV).";
    ...
    SessionDataBox (int _x, int _y, int _w, int _h, int _padding, int _dataSource, int output, String textfieldName) {
        ...
        createODFButton("odfButton", "Cognisync", dataLogger.getDataLoggerOutputFormat(), x + padding, y + padding*2 + 18 + 58, (w-padding*3)/2, 24);
    ...
    private void createODFButton(String name, String text, int dataLoggerFormat, int _x, int _y, int _w, int _h) {
        ...
        outputODF.setDescription("Set GUI data output to Cognisync Data Format (.txt). A new file will be made in the session folder when the data stream is paused or max file duration is reached.");
    }
...
class RecentPlaybackBox {
    ...
    private void getRecentPlaybackFiles() {
        ...
        if (!f.exists()) {
            println("Cognisync_GUI::Control Panel: Playback history file not found.");
        ...
        try {
            ...
            println("Cognisync_GUI::Control Panel: Other error! Please submit an issue on Github and share this console log.");
...
class ESP32Box {
    ...
    private ControlP5 esp32Box_cp5;
    
    // Protocol Selection Components
    private Button esp32ProtocolSerial;
    private Button esp32ProtocolWifi;
    private Button esp32ProtocolBluetooth;
    private String selectedProtocol = "Serial";
    
    // Serial/USB Components
    private MenuList esp32SerialPortList;
    private Button refreshSerialPorts;
    
    // WiFi Components  
    private Button wifiDynamicIP;
    private Button wifiStaticIP;
    private Textfield staticIPAddressTF;
    private MenuList esp32WifiList;
    private Button refreshWifiDevices;
    private boolean isWifiSearching = false;
    private boolean useStaticIP = false;
    
    // Bluetooth Components
    private MenuList esp32BluetoothList;
    private Button scanBluetoothDevices;
    private boolean isBluetoothScanning = false;
    
    ESP32Box(int _x, int _y, int _w, int _h, int _padding) {
        x = _x;
        y = _y;
        w = _w;
        h = 260; // Professional height similar to WiFi box
        padding = _padding;
        
        // Initialize ControlP5
        esp32Box_cp5 = new ControlP5(ourApplet);
        esp32Box_cp5.setGraphics(ourApplet, 0, 0);
        esp32Box_cp5.setAutoDraw(false);
        
        // Create all UI components
        createProtocolSelectionButtons();
        createSerialComponents();
        createWiFiComponents();
        createBluetoothComponents();
        
        // Set default state
        esp32ProtocolSerial.setOn();
        updateProtocolVisibility();
        refreshSerialPortList();
    }
    
    private void createProtocolSelectionButtons() {
        int buttonWidth = (w - padding * 4) / 3;
        int protocolY = y + padding * 2 + 16;
        
        esp32ProtocolSerial = createESP32Button("esp32ProtocolSerial", "SERIAL (USB)", 
            x + padding, protocolY, buttonWidth, 24);
        esp32ProtocolSerial.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                selectedProtocol = "Serial";
                esp32ProtocolSerial.setOn();
                esp32ProtocolWifi.setOff();
                esp32ProtocolBluetooth.setOff();
                updateProtocolVisibility();
                refreshSerialPortList();
            }
        });
        
        esp32ProtocolWifi = createESP32Button("esp32ProtocolWifi", "WIFI (NETWORK)", 
            x + padding * 2 + buttonWidth, protocolY, buttonWidth, 24);
        esp32ProtocolWifi.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                selectedProtocol = "WiFi";
                esp32ProtocolSerial.setOff();
                esp32ProtocolWifi.setOn();
                esp32ProtocolBluetooth.setOff();
                updateProtocolVisibility();
                refreshWiFiDeviceList();
            }
        });
        
        esp32ProtocolBluetooth = createESP32Button("esp32ProtocolBluetooth", "BLUETOOTH (BLE)", 
            x + padding * 3 + buttonWidth * 2, protocolY, buttonWidth, 24);
        esp32ProtocolBluetooth.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                selectedProtocol = "Bluetooth";
                esp32ProtocolSerial.setOff();
                esp32ProtocolWifi.setOff();
                esp32ProtocolBluetooth.setOn();
                updateProtocolVisibility();
    ...
    private void createSerialComponents() {
        // Serial port list
        esp32SerialPortList = new MenuList(esp32Box_cp5, "esp32SerialPorts", w - padding * 2, 72, p3);
        esp32SerialPortList.setPosition(x + padding, y + padding * 5 + 16 + 24);
        esp32SerialPortList.addCallback(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                if (theEvent.getAction() == ControlP5.ACTION_BROADCAST) {
                    Map bob = esp32SerialPortList.getItem(int(esp32SerialPortList.getValue()));
                    openBCI_portName = (String)bob.get("subline");
                    output("ESP32: Selected Serial Port " + openBCI_portName);
                }
            }
        });
        
        // Refresh button for serial ports
        refreshSerialPorts = createESP32Button("refreshESP32SerialPorts", "REFRESH PORTS",
            x + padding, y + padding * 7 + 72 + 16 + 24, w - padding * 2, 24);
        refreshSerialPorts.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                refreshSerialPortList();
            }
        });
    ...
    private void createWiFiComponents() {
        int buttonWidth = (w - padding * 3) / 2;
        int wifiButtonY = y + padding * 4 + 16 + 24;
        
        // Dynamic/Static IP selection buttons
        wifiDynamicIP = createESP32Button("esp32WifiDynamic", "DYNAMIC IP", 
            x + padding, wifiButtonY, buttonWidth, 24);
        wifiDynamicIP.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                useStaticIP = false;
                wifiDynamicIP.setOn();
                wifiStaticIP.setOff();
        ...
        wifiStaticIP = createESP32Button("esp32WifiStatic", "STATIC IP", 
            x + padding * 2 + buttonWidth, wifiButtonY, buttonWidth, 24);
        wifiStaticIP.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                useStaticIP = true;
                wifiDynamicIP.setOff();
                wifiStaticIP.setOn();
                updateWiFiVisibility();
            }
        });
        
        // WiFi device list for dynamic IP
        esp32WifiList = new MenuList(esp32Box_cp5, "esp32WifiDevices", w - padding * 2, 72, p3);
        esp32WifiList.setPosition(x + padding, y + padding * 5 + 16 + 24 + 24);
        esp32WifiList.addCallback(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                if (theEvent.getAction() == ControlP5.ACTION_BROADCAST) {
                    Map bob = esp32WifiList.getItem(int(esp32WifiList.getValue()));
                    wifi_portName = (String)bob.get("headline");
                    wifi_ipAddress = (String)bob.get("subline");
                    output("ESP32: Selected WiFi Device " + wifi_portName + " at " + wifi_ipAddress);
                }
            }
        });
        
        // Refresh button for WiFi devices
        refreshWifiDevices = createESP32Button("refreshESP32WifiDevices", "SEARCH DEVICES",
            x + padding, y + padding * 7 + 72 + 16 + 24 + 24, w - padding * 2, 24);
        refreshWifiDevices.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                refreshWiFiDeviceList();
            }
        });
        
        // Static IP text field
        staticIPAddressTF = esp32Box_cp5.addTextfield("esp32StaticIPAddress")
            .setPosition(x + padding, y + padding * 5 + 16 + 24 + 24 + 10)
            .setCaptionLabel("")
            .setSize(w - padding * 2, 26)
            .setFont(f2)
            .setFocus(false)
        ...
        // Set default to dynamic IP
        wifiDynamicIP.setOn();
    }
    
    private void createBluetoothComponents() {
        // Bluetooth device list
        esp32BluetoothList = new MenuList(esp32Box_cp5, "esp32BluetoothDevices", w - padding * 2, 72, p3);
        esp32BluetoothList.setPosition(x + padding, y + padding * 5 + 16 + 24);
        esp32BluetoothList.addCallback(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                if (theEvent.getAction() == ControlP5.ACTION_BROADCAST) {
                    Map bob = esp32BluetoothList.getItem(int(esp32BluetoothList.getValue()));
                    String deviceName = (String)bob.get("headline");
                    output("ESP32: Selected Bluetooth Device " + deviceName);
                }
            }
        });
        
        // Scan button for Bluetooth devices
        scanBluetoothDevices = createESP32Button("scanESP32BluetoothDevices", "SCAN DEVICES",
            x + padding, y + padding * 7 + 72 + 16 + 24, w - padding * 2, 24);
        scanBluetoothDevices.onRelease(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                scanForBluetoothDevices();
            }
        });
    ...
    private Button createESP32Button(String name, String text, int _x, int _y, int _w, int _h) {
        Button b = createButton(esp32Box_cp5, name, text, _x, _y, _w, _h);
        b.setSwitch(true);
        return b;
    }
    
    private void refreshSerialPortList() {
        esp32SerialPortList.items.clear();
        
        Thread thread = new Thread() {
            public void run() {
                refreshSerialPorts.getCaptionLabel().setText("SEARCHING...");
                ...
                if (portNames.length == 0) {
                    esp32SerialPortList.addItem("No ports found", "N/A", "");
                }
                
                esp32SerialPortList.updateMenu();
                refreshSerialPorts.getCaptionLabel().setText("REFRESH PORTS");
            }
        };
        thread.start();
    }
    
    private void refreshWiFiDeviceList() {
        esp32WifiList.items.clear();
        
        Thread thread = new Thread() {
            public void run() {
                refreshWifiDevices.getCaptionLabel().setText("SEARCHING...");
                isWifiSearching = true;
                
                try {
                    // Simulate ESP32 WiFi device discovery
                    Thread.sleep(2000);
                    
                    esp32WifiList.addItem("ESP32-BrainFlow-01", "192.168.4.1", "");
                    esp32WifiList.addItem("ESP32-BrainFlow-02", "192.168.4.2", "");
                    esp32WifiList.addItem("OpenBCI-ESP32-v3", "192.168.1.100", "");
                    esp32WifiList.addItem("ESP32-DevKit-Pro", "192.168.1.101", "");
                    
                    esp32WifiList.updateMenu();
                } catch (InterruptedException e) {
                    println("WiFi scan interrupted");
                }
                
                refreshWifiDevices.getCaptionLabel().setText("SEARCH DEVICES");
    ...
    private void scanForBluetoothDevices() {
        esp32BluetoothList.items.clear();
        
        Thread thread = new Thread() {
            public void run() {
                scanBluetoothDevices.getCaptionLabel().setText("SCANNING...");
                isBluetoothScanning = true;
                
                try {
                    Thread.sleep(3000); // Simulate Bluetooth scan time
                    
                    esp32BluetoothList.addItem("ESP32-BLE-Audio", "ESP32-Audio", "High-quality audio streaming");
                    esp32BluetoothList.addItem("OpenBCI-ESP32-v2", "ESP32-v2", "OpenBCI compatible ESP32");
                    esp32BluetoothList.addItem("ESP32-DevKit-C", "ESP32-Dev", "Development board");
                    esp32BluetoothList.addItem("ESP32-WROOM-32", "ESP32-WROOM", "Standard ESP32 module");
                    esp32BluetoothList.addItem("BrainFlow-ESP32", "BF-ESP32", "BrainFlow ESP32 device");
                    
                    esp32BluetoothList.updateMenu();
                } catch (InterruptedException e) {
                    println("Bluetooth scan interrupted");
                }
                
                scanBluetoothDevices.getCaptionLabel().setText("SCAN DEVICES");
    ...
    private void updateProtocolVisibility() {
        // Hide all protocol-specific components
        hideAllComponents();
        
        // Show components based on selected protocol
        if (selectedProtocol.equals("Serial")) {
            esp32SerialPortList.setVisible(true);
            refreshSerialPorts.setVisible(true);
        } else if (selectedProtocol.equals("WiFi")) {
            wifiDynamicIP.setVisible(true);
            wifiStaticIP.setVisible(true);
            updateWiFiVisibility();
        } else if (selectedProtocol.equals("Bluetooth")) {
            esp32BluetoothList.setVisible(true);
            scanBluetoothDevices.setVisible(true);
        }
    }
    
    private void updateWiFiVisibility() {
        if (useStaticIP) {
            esp32WifiList.setVisible(false);
            refreshWifiDevices.setVisible(false);
            staticIPAddressTF.setVisible(true);
        } else {
            esp32WifiList.setVisible(true);
            refreshWifiDevices.setVisible(true);
            staticIPAddressTF.setVisible(false);
        }
    }
    ...
    private void hideAllComponents() {
        // Serial components
        esp32SerialPortList.setVisible(false);
        refreshSerialPorts.setVisible(false);
        
        // WiFi components
        wifiDynamicIP.setVisible(false);
        wifiStaticIP.setVisible(false);
        esp32WifiList.setVisible(false);
        refreshWifiDevices.setVisible(false);
        staticIPAddressTF.setVisible(false);
        
        // Bluetooth components
        esp32BluetoothList.setVisible(false);
        scanBluetoothDevices.setVisible(false);
    }
    ...
    public void draw() {
        ...
        textFont(h3, 16);
        textAlign(LEFT, TOP);
        text("ESP32 COMMUNICATION", x + padding, y + padding);
        popStyle();
        
        // Draw section headers and status based on selected protocol
        if (selectedProtocol.equals("Serial")) {
            drawSerialInterface();
        } else if (selectedProtocol.equals("WiFi")) {
            drawWiFiInterface();
        } else if (selectedProtocol.equals("Bluetooth")) {
            drawBluetoothInterface();
        }
        
        esp32Box_cp5.draw();
    }
    
    private void drawSerialInterface() {
        pushStyle();
        fill(OPENBCI_DARKBLUE);
        textFont(h3, 14);
        textAlign(LEFT, TOP);
        text("SERIAL PORTS", x + padding, y + padding * 4 + 16 + 24 + 5);
        
        // Connection status
        String connectionInfo = "USB CONNECTION: ";
        if (esp32SerialPortList.activeItem >= 0) {
            Map item = esp32SerialPortList.getItem(esp32SerialPortList.activeItem);
            connectionInfo += (String)item.get("subline");
        } else {
            connectionInfo += "None Selected";
        }
        
        textFont(h3, 12);
        text(connectionInfo, x + padding, y + h - padding - 15);
        popStyle();
    }
    
    private void drawWiFiInterface() {
        pushStyle();
        fill(OPENBCI_DARKBLUE);
        textFont(h3, 14);
        textAlign(LEFT, TOP);
        ...
        if (useStaticIP) {
            text("STATIC IP ADDRESS", x + padding, y + padding * 4 + 16 + 24 + 24 + 5);
            
            // Position static IP field properly
            staticIPAddressTF.setPosition(x + padding, y + padding * 5 + 16 + 24 + 24 + 20);
            
            // Connection status
            textFont(h3, 12);
            text("WIFI IP: " + staticIPAddressTF.getText(), x + padding, y + h - padding - 15);
        } else {
            text("WIFI DEVICES", x + padding, y + padding * 4 + 16 + 24 + 24 + 5);
            
            // Show scanning indicator
            if (isWifiSearching) {
                image(loadingGIF_blue, x + w - padding - 30, y + padding * 7 + 72 + 16 + 24 + 24 + 4, 20, 20);
            }
            
            // Connection status
            String connectionInfo = "WIFI DEVICE: ";
            if (esp32WifiList.activeItem >= 0) {
                Map item = esp32WifiList.getItem(esp32WifiList.activeItem);
                connectionInfo += (String)item.get("headline") + " (" + (String)item.get("subline") + ")";
            } else {
                connectionInfo += "None Selected";
            }
    ...
    private void drawBluetoothInterface() {
        pushStyle();
        fill(OPENBCI_DARKBLUE);
        textFont(h3, 14);
        textAlign(LEFT, TOP);
        text("BLUETOOTH DEVICES", x + padding, y + padding * 4 + 16 + 24 + 5);
        
        // Show scanning indicator
        if (isBluetoothScanning) {
            image(loadingGIF_blue, x + w - padding - 30, y + padding * 7 + 72 + 16 + 24 + 4, 20, 20);
        }
        
        // Connection status
        String connectionInfo = "BLE DEVICE: ";
        if (esp32BluetoothList.activeItem >= 0) {
            Map item = esp32BluetoothList.getItem(esp32BluetoothList.activeItem);
            connectionInfo += (String)item.get("headline");
        } else {
            connectionInfo += "None Selected";
        }
    ...
    public String getSelectedPort() {
        if (esp32SerialPortList.activeItem >= 0) {
            Map item = esp32SerialPortList.getItem(esp32SerialPortList.activeItem);
            return (String)item.get("subline");
        }
    ...
    public String getWiFiIPAddress() {
        if (useStaticIP) {
            return staticIPAddressTF.getText();
        } else if (esp32WifiList.activeItem >= 0) {
            Map item = esp32WifiList.getItem(esp32WifiList.activeItem);
            return (String)item.get("subline");
        }
        return "192.168.4.1"; // Default fallback
    }
    
    public String getSelectedBluetoothDevice() {
        if (esp32BluetoothList.activeItem >= 0) {
            Map item = esp32BluetoothList.getItem(esp32BluetoothList.activeItem);
            return (String)item.get("headline");
        }
        return "";
    }
    ...
    public String getConnectionInfo() {
        switch (selectedProtocol) {
            ...
            case "Bluetooth":
                return getSelectedBluetoothDevice();
            default:
                return "";
        }
    }
...
class PlaybackFileBox {
    ...
    PlaybackFileBox(int _x, int _y, int _w, int _h, int _padding) {
        ...
        createSelectPlaybackFileButton("selectPlaybackFileControlPanel", "SELECT Cognisync PLAYBACK FILE", x + padding, y + padding*2 + titleH, w - padding*2, buttonH);
    ...
    private void createSelectPlaybackFileButton(String name, String text, int _x, int _y, int _w, int _h) {
        ...
        selectPlaybackFile.setDescription("Click to open a dialog box to select a Cognisync playback file (.txt or .csv).");
    }
    ...
    private void createSampleDataButton(String name, String text, int _x, int _y, int _w, int _h) {
        ...
        sampleDataButton.setDescription("Click to open the folder containing Cognisync GUI Sample Data.");
    }
};
        ...
        sdList.addCallback(new CallbackListener() {
            public void controlEvent(CallbackEvent theEvent) {
                if (theEvent.getAction() == ControlP5.ACTION_BROADCAST) {
                    ...
                    String outputString = "Cognisync microSD Setting = " + cyton_sdSetting.getName();
