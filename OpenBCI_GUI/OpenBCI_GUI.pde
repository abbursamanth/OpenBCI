...
//
//   Modified (v1.0) Santosh, August 2025
//
...
String localGUIVersionString = "v1.1 beta";
String localGUIVersionDate = "August 2024";
String guiLatestVersionGithubAPI = "https://api.github.com/repos/Cognisync.tech/Cognisync_GUI/releases/latest";
String guiLatestReleaseLocation = "https://github.com/Cognisync.tech/Cognisync_GUI/releases/latest";
...
final int DATASOURCE_ESP32 = 6; // Add this line
...
final color OPENBCI_DARKBLUE = color(12, 7, 38);      // Replaced with #0C0726
final color OPENBCI_BLUE = color(26, 26, 74);      // Replaced with #1A1A4A
...
void setup() {
    ...
    // Assuming you have Roboto-Bold.ttf and Roboto-Regular.ttf in your fonts folder
    p0 = createFont("fonts/Roboto-Bold.ttf", 24);
    p1 = createFont("fonts/Roboto-Regular.ttf", 20);
    p2 = createFont("fonts/Roboto-Regular.ttf", 18);
    p3 = createFont("fonts/Roboto-Regular.ttf", 16);
    p15 = createFont("fonts/Roboto-Regular.ttf", 15);
    p4 = createFont("fonts/Roboto-Regular.ttf", 14);
    p13 = createFont("fonts/Roboto-Regular.ttf", 13);
    p5 = createFont("fonts/Roboto-Regular.ttf", 12);
    p6 = createFont("fonts/Roboto-Regular.ttf", 10);

    cog = loadImage("Cognisync_logo_dark.png");

    // check if the current directory is writable
    File dummy = new File(sketchPath());
    if (!dummy.canWrite()) {
        showStartupError = true;
        startupErrorMessage = "Cognisync GUI was launched from a read-only location.\n\n" +
            ...
            "If this error persists, contact the Cognisync team for support.";
        return; // early exit
    }
    ...
    println("Welcome to the CogniSync GUI!"); //Welcome line.
    println("For more information, please visit: https://cognisync.tech/");
...
void delayedSetup() {
    ...
    // This logo will be used on light backgrounds
    logo_black = loadImage("Cognisync_logo_dark.png");
    // This can be a primary brand color logo or the same as above
    logo_blue = loadImage("Cognisync_logo_dark.png");
    // This logo will be used on dark backgrounds
    logo_white = loadImage("Cognisync_logo_light.png");
    ...
    synchronized(this) {
        ...
        println("Cognisync_GUI::Setup: Setup is complete!");
    }
    ...
    if (!isAdminUser() || isElevationNeeded()) {
        outputError("Cognisync_GUI: This application is not being run with Administrator access. This could limit the ability to connect to devices or read/write files.");
    }
}
...
void initSystem() {
    ...
    verbosePrint("Cognisync_GUI: initSystem: -- Init 0 -- ");
    ...
    switch (eegDataSource) {
        ...
        case DATASOURCE_SYNTHETIC:
            ...
            println("Cognisync_GUI: Init session using Synthetic data source");
            break;
        case DATASOURCE_PLAYBACKFILE:
            if (!playbackData_fname.equals("N/A")) {
                currentBoard = getDataSourcePlaybackClassFromFile(playbackData_fname);
                println("Cognisync_GUI: Init session using Playback data source");
            } else {
        ...
        case DATASOURCE_ESP32:
            // Use Cyton board with Arduino serial port - BrainFlow handles the protocol
            ...
            println("CogniSync_GUI: Init session using Arduino on " + arduinoPort + " via BrainFlow Cyton protocol");
            break;
        default:
            break;
    }
    ...
    if (eegDataSource == DATASOURCE_CYTON) {
        println("Cognisync_GUI: Configuring Cyton Channel Count...");
        if (currentBoard instanceof BoardCytonSerial) {
            ...
            if (res.getValue().startsWith("daisy removed")) {
                println("Cognisync_GUI: Daisy is physically attached, using Cyton 8 Channels instead.");
            }
        ...
        if (guiSettings.getShowCytonSmoothingPopup()) {
            println("Cognisync_GUI: Showing Cyton FTDI Buffer Fix Popup");
            String popupTitle = "Cyton FTDI Buffer Fix Info";
            String popupString = "The default settings for the Cyton Dongle driver can make data appear \"choppy.\" Visit the Cognisync Docs to learn how to fix this. For now, the GUI will \"smooth\" the data for you.";
            String popupButtonText = "View Fix";
            String popupButtonURL;
            if (isMac()) {
                popupButtonURL = "https://docs.cognisync.com/Troubleshooting/FTDI_Fix_Mac/";
                PopupMessage msg = new PopupMessage(popupTitle, popupString, popupButtonText, popupButtonURL);
            } else if (isLinux()){
                popupButtonURL = "https://docs.cognisync.com/Troubleshooting/FTDI_Fix_Linux/";
                PopupMessage msg = new PopupMessage(popupTitle, popupString, popupButtonText, popupButtonURL);
            }
    ...
    verbosePrint("Cognisync_GUI: initSystem: Initializing core data objects");
    initCoreDataObjects();

    verbosePrint("Cognisync_GUI: initSystem: -- Init 1 -- " + millis());
    verbosePrint("Cognisync_GUI: initSystem: Initializing FFT data objects");
    initFFTObjectsAndBuffer();

    verbosePrint("Cognisync_GUI: initSystem: -- Init 2 -- " + millis());
    verbosePrint("Cognisync_GUI: initSystem: Closing ControlPanel...");

    controlPanel.close();
    topNav.controlPanelCollapser.setOff();

    verbosePrint("Cognisync_GUI: initSystem: -- Init 3 -- " + millis());
    ...
    verbosePrint("Cognisync_GUI: initSystem: -- Init 4 -- " + millis());
    ...
    verbosePrint("Cognisync_GUI: initSystem: -- Init 5 -- " + millis());
...
void haltSystem() {
    if (!systemHasHalted) { //prevents system from halting more than once
        println("Cognisync_GUI: haltSystem: Halting system for reconfiguration of settings...");
        ...
        if (w_networking != null && w_networking.getNetworkActive()) {
            ...
            println("Cognisync_GUI: haltSystem: Network streams stopped");
        }
...
void systemDraw() { //for drawing to the screen
    ...
    surface.setTitle("CogniSync GUI " + localGUIVersionString + " - " + localGUIVersionDate + " - " + int(frameRate) + " fps");
}
...
void systemInitSession() {
    if (midInitCheck2) {
        println("Cognisync_GUI: Start session. Calling initSystem().");
        try {
...
void updateToNChan(int _nchan) {
    ...
    println("Cognisync_GUI: Channel count set to " + str(nchan));
}
...
void introAnimation() {
    ...
    if (millis() >= settings.introAnimationInit) {
        ...
        // --- New code to draw logo with correct aspect ratio ---
        // Define a desired width for the logo
        float logoDisplayWidth = width / 6.0;
        // Calculate the correct height to maintain the logo's shape
        float logoDisplayHeight = logoDisplayWidth * (float)cog.height / (float)cog.width;
        // Draw the image with the correct proportions
        image(cog, width/2, height/2, logoDisplayWidth, logoDisplayHeight);
        // --- End of new code ---
        textFont(p3, 16);
        textLeading(24);
        fill(255, transparency); // 255 is white
        textAlign(CENTER, CENTER);
        String displayVersion = "CogniSync GUI " + localGUIVersionString;
